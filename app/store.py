...
class InMemoryStore:
    def __init__(self, db_path: str = "interflow.db") -> None:
        ...
        self.intents: Dict[str, IntentProfile] = {}
        ...
        self.case_index = defaultdict(lambda: {
            "stakeholders": [], "evidence": [], "hypotheses": [], "interventions": [], "intents": [], "validations": [], "audit": [], "outcomes": []
        })
        self._load_from_db()
    ...
    def _load_from_db(self) -> None:
        ...
        for raw in self.db.list_namespace("intents"):
            intent = IntentProfile(**raw)
            self.intents[intent.id] = intent
            self.link(intent.case_id, "intents", intent.id)
    ...
    def save_intent(self, intent: IntentProfile) -> None:
        self.intents[intent.id] = intent
        self.link(intent.case_id, "intents", intent.id)
        self.db.upsert("intents", intent.id, intent.model_dump(mode="json"), intent.case_id)
    ...
    def get_case_bundle(self, case_id: str) -> Dict[str, List[dict]]:
        ...
        return {
            ...
            "intents": [self.intents[i].model_dump(mode="json") for i in idx["intents"] if i in self.intents],
