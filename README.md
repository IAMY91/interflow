...
The practitioner experience uses an **intent-to-intervention** flow: a human
states the current reality, desired state, guiding intent, success signals, and
vitality guardrails before agents interpret evidence. Intent is stored as
human-authored context—not inferred as psychological truth—and is available at
`PUT /cases/{case_id}/intent`.
